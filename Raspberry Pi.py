...
def myCommandCallback(cmd):
    if cmd.command == "display":
        ...
        if command == "on":
            ...
            sense.set_pixels(getPixelMap("red",temp))
            # TODO insert your code here
            pass
        elif command == "off":
            temp = sense.get_temperature()
            sense.set_pixels(getPixelMap("green",temp))
            # TODO insert your code here
            pass

try:
    gatewayOptions = {"org": "t2b8h3", "type": "raspberryGateway", "id": "b827eba9d2bd", "auth-method": "token", "auth-token": "shamin12"}
    gatewayCli = ibmiotf.gateway.Client(gatewayOptions)

    gatewayCli.connect()
    gatewayCli.deviceCommandCallback = myCommandCallback 
    gatewayCli.subscribeToDeviceCommands(deviceType='raspberrySensor', deviceId='senb827ebfc87e8', command='display',format='json',qos=2)

    while True:
        temp = sense.get_temperature() # TODO insert your code here
        myData = {'d' : {'temperature' : temp}} # TODO insert your code here
        gatewayCli.publishDeviceEvent("raspberrySensor", "senb827ebfc87e8", "event", "json", myData, qos=1 )
        time.sleep(2)
